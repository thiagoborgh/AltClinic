...
router.get('/view/:imagemId', authenticateToken, async (req, res) => {
  try {
    ...
    const imagem = db.prepare(`
      SELECT arquivo_path FROM prontuario_imagem WHERE id = ?
    `).get(imagemId);
    ...
    // Verificar se arquivo existe e servir
    const caminhoArquivo = imagem.arquivo_path;
...
router.delete('/:imagemId', authenticateToken, async (req, res) => {
  try {
    ...
    try {
      await fs.unlink(imagem.arquivo_path);
    } catch (error) {
...
router.post('/comparacao', authenticateToken, async (req, res) => {
  try {
    ...
    // Buscar imagens
    const imagens = db.prepare(`
      SELECT id, categoria, descricao, data_imagem, arquivo_path
      FROM prontuario_imagem
      WHERE id IN (${imagemIds.map(() => '?').join(',')})
    `).all(...imagemIds);
