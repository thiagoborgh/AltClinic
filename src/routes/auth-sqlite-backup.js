...
router.post('/login', async (req, res) => {
  try {
    ...
    // Buscar TODOS os tenants do usuário baseado no email
    let matchingTenants = [];
    let invalidPasswordTenant = null;

    try {
      const masterDb = multiTenantDb.getMasterDb();
      const tenants = masterDb.prepare('SELECT id, nome, slug FROM tenants WHERE status IN (\'active\', \'trial\')').all();

      // Tentar autenticar em cada tenant e coletar todos onde o usuário existe
      for (const tenant of tenants) {
        try {
          const authResult = await UsuarioModel.authenticate(email, senha, tenant.id);
          if (authResult.success) {
            // Usuário autenticado com sucesso neste tenant
            matchingTenants.push({
              tenant: tenant,
              user: authResult.user
            });
          } else if (authResult.error === 'INVALID_PASSWORD' && !invalidPasswordTenant) {
            // Usuário encontrado mas senha incorreta
            invalidPasswordTenant = tenant;
          }
        } catch (authError) {
      ...
      // Se não encontrou nenhum tenant mas encontrou com senha errada
      if (matchingTenants.length === 0 && invalidPasswordTenant) {
        return res.status(401).json({
          success: false,
      ...
      // Se encontrou múltiplos tenants, retornar para seleção
      if (matchingTenants.length > 1) {
        return res.json({
          success: true,
          multipleTenants: true,
          tenants: matchingTenants.map(mt => ({
            id: mt.tenant.id,
            nome: mt.tenant.nome,
            slug: mt.tenant.slug,
            userRole: mt.user.role
          })),
          message: 'Múltiplas clínicas encontradas. Selecione uma para continuar.'
        });
      }

      // Se encontrou apenas 1 tenant, fazer login automático
      const { tenant: tenantInfo, user: usuario } = matchingTenants[0];
      const foundTenantId = tenantInfo.id;
    } catch (dbError) {
...
// � LOGIN COM TENANT ESPECÍFICO (quando usuário escolhe entre múltiplos)
router.post('/login/select-tenant', async (req, res) => {
  try {
    ...
    const masterDb = multiTenantDb.getMasterDb();
    const tenant = masterDb.prepare('SELECT id, nome, slug FROM tenants WHERE id = ? AND status IN (\'active\', \'trial\')').get(tenantId);
    ...
    const usuario = authResult.user;
    const isFirstAccess = usuario.status === 'pending_first_access' || usuario.email_verified_at === null;
